...
const authors=b=>(b.authorNames||String(b.authors||'').split(',')).map(normalise).filter(a=>a&&a!=='unknown author');
export function rankBooks(candidates,seed,subjects,{newAuthor=true,era='any'}={}){
 const wanted=new Set(subjects.map(normalise));const seen=new Set();const seedAuthors=new Set(authors(seed));
 ...
 return {...b,matches};
 }).filter(b=>b.matches.length&&(!newAuthor||!authors(b).some(a=>seedAuthors.has(a)))&&(era==='any'||(Number.isFinite(b.year)&&(era==='recent'?b.year>=2000:b.year<2000))))
